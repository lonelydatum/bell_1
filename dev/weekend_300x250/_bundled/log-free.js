...
var data = [{ read: read[0], y: -900 }, { read: read[1], y: -500 }, { read: read[2], y: -100 }];

function start() {

	var tl = (0, _commonJsCommonJs.init)(data);

	tl.to(".t1", .3, { opacity: 0 }, "+=" + read[0]);

	tl.from([".t2"], _commonJsCommonJs.fadeTime, { opacity: 0 });

	tl.to(".t2", .3, { opacity: 0 }, "+=" + read[1]);
	tl.from(".t3", _commonJsCommonJs.fadeTime, { opacity: 0 });
